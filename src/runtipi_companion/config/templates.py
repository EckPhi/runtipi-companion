...
backup:
  ...
  remotes:
    - name: backblaze
      ...
      # Optional: use an existing rclone Remote Control API instead of a
      # local rclone binary. Set all three fields together. The password is
      # read from the environment, never stored in this file.
      api_url: null            # e.g. http://rclone:5533
      api_username: null
      api_password_env: null   # e.g. RCLONE_API_PASSWORD
      schedules:
        daily:
          retention: 14
        weekly:
  ...
  # Per-app overrides, keyed by app id. Same fields can also be set as
  # `runtipi-companion.backup.*` docker labels on the app's own container
  # (keep-running/exclude/pre-command/post-command/restore-command,
  # comma-separated for exclude) -- handy for setting defaults across many
  # apps without editing this file. Whatever's set here wins over a label,
  # field by field.
  #
  # Example: QuestDB's own zero-downtime checkpoint mechanism
  # (https://questdb.com/docs/operations/backup/) instead of stopping it.
  # CHECKPOINT RELEASE must run whether the backup succeeded or failed --
  # post_backup_command always runs, unlike pre_backup_command/restore_command.
  # app_settings:
  #   questdb:
  #     keep_running: true   # don't stop the container for this app's backups
  #     exclude_patterns: []   # regex, matched against each archived member's path
  #     pre_backup_command: >-
